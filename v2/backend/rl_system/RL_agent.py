...
class RLAgent:
    ...
    def update(self, state, action, reward, next_state):
        ...
        next_Q_vals = [self.get_q_value(next_state, a) for a in self.actions]
...
if __name__ == "__main__":
    ...
    print("Before update, Q({}, {}): {:.4f}".format(state, action, agent.get_q_value(state, action)))
    agent.update(state, action, reward, next_state)
    print("After update, Q({}, {}): {:.4f}".format(state, action, agent.get_q_value(state, action)))
